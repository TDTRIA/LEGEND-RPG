...
(() => {
  ...
  async function createAccount(email, password, displayName){
    const c = client();
    if(!c?.auth?.signUp) return { ok:false, message:'Supabase is not ready yet.' };
    ...
    try {
      ...
      return { ok:false, message:err.message || 'Could not create account.' };
    }
  }

  async function signIn(email, password){
    const c = client();
    if(!c?.auth?.signInWithPassword) return { ok:false, message:'Supabase is not ready yet.' };
    if(!email || !password) return { ok:false, message:'Enter an email and password.' };
    try {
  ...
  async function signOut(){
    const c = client();
    if(!c?.auth?.signOut) return { ok:false, message:'Supabase is not ready yet.' };
    try {
      const result = await c.auth.signOut();
      if(result.error) throw result.error;
      return { ok:true, message:'Signed out. Local traveler save kept.' };
    } catch(err){
      return { ok:false, message:err.message || 'Could not sign out.' };
    }
  }
