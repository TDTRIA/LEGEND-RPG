...
(() => {
    ...
    overlay.innerHTML = `
      ...
      <section class="inv-panel" role="dialog" aria-label="Inventory">
        <header class="inv-head">
          <div><div class="inv-kicker">Recovered Pack v0.6.5</div><h2>${esc(p.username || 'Hero')}'s Inventory</h2><p>Better item grouping, equipment summary, quest items, trade value, and item purpose.</p></div>
          <button class="inv-close" id="closeInventory">×</button>
        </header>
  ...
  function injectInventoryButton(){
    ...
    if(!/Character Sheet/i.test(text)) return;
    const actions = document.querySelector('.actions');
    if(!actions) return;
    const btn = document.createElement('button');
    btn.className = 'btn primary';
    btn.id = 'openBetterInventory';
    btn.textContent = 'Open Better Inventory';
  ...
  window.LegendInventoryV065 = { open: renderInventory };
})();
