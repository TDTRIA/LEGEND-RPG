// LEGEND v0.6.4 — Travel System Updated with NPC Reminders
(() => {
  ...
  function applyClassBonus(event,p){
    ...
    switch(p.className){
      case 'Ranger': if(event.check==='survival' && Math.random()<0.5){ p.fatigue = Math.max(0,(p.fatigue||0)-1); } break;
      case 'Warrior': if(event.check==='strength' && Math.random()<0.5){ p.hp = (p.hp||100)+5; } break;
      case 'Mage': if(event.check==='magic'||event.check==='lore'){ p.xp = (p.xp||0)+5; } break;
      case 'Thief': if(event.check==='luck'){ p.gold = (p.gold||0)+10; } break;
      case 'Skiller': if(event.check==='craft'||event.check==='survival'){ p.inventory.roadToken=(p.inventory.roadToken||0)+1; } break;
    }
  }
  ...
  function exploreOldRoad(){
    ...
    if(p.npcTalk && p.npcTalk.sella>0){
      message+=' Sella reminds you: Keep an eye on your fatigue and supplies.';
    }
